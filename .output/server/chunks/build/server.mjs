import { version, unref, inject, defineComponent, useSlots, h, Fragment, defineAsyncComponent, computed, ref, provide, shallowReactive, watch, Suspense, nextTick, Transition, hasInjectionContext, getCurrentInstance, reactive, toRaw, mergeProps, useSSRContext, createApp, effectScope, getCurrentScope, onErrorCaptured, onServerPrefetch, createVNode, resolveDynamicComponent, toRef, shallowRef, isReadonly, withCtx, isRef, isShallow, isReactive, onScopeDispose } from 'vue';
import { $ as $fetch, k as hasProtocol, l as isScriptProtocol, m as joinURL, w as withQuery, n as sanitizeStatusCode, o as getContext, p as createHooks, e as createError$1, q as toRouteMatcher, r as createRouter$1 } from '../_/nitro.mjs';
...
const _routes = [
  {
    name: "index",
    path: "/",
    component: () => import('./index-CU_4t286.mjs')
  },
  {
    name: "services",
    path: "/services",
    component: () => import('./services-CXkXymVV.mjs')
  }
];
...
function useMotionControls(motionProperties, variants = {}, { motionValues, push, stop } = useMotionTransitions()) {
  ...
  const apply = (variant) => {
    ...
    async function waitForComplete() {
      ...
      (_b = (_a = variant.transition) == null ? undefined : _a.onComplete) == null ? undefined : _b.call(_a);
    }
...
function directive(variants, isPreset = false) {
  ...
  return {
    ...
    getSSRProps(binding, node) {
      let { initial: bindingInitial } = binding.value || node && (node == null ? undefined : node.props) || {};
      bindingInitial = unref(bindingInitial);
      const initial = defu({}, (variants == null ? undefined : variants.initial) || {}, bindingInitial || {});
...
const MotionGroupComponent = defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      var _a;
      const style = variantToStyle(motionConfig.value.initial || {});
      const nodes = ((_a = slots.default) == null ? undefined : _a.call(slots)) || [];
...
const MotionPlugin = {
  install(app, options) {
    ...
    app.provide(CUSTOM_PRESETS, options == null ? undefined : options.directives);
...
const layouts = {
  default: defineAsyncComponent(() => import('./default-DmRqzJih.mjs').then((m) => m.default || m))
};
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const _Error404 = defineAsyncComponent(() => import('./error-404-v_cjt_q0.mjs'));
    const _Error = defineAsyncComponent(() => import('./error-500-BxSCrb8T.mjs'));
